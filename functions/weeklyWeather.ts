...
export const fetchWeeklyWeatherData = async (request, locationInput) => {
	const { city, state, zipCode, lat, lon } = locationInput;
	...
	weatherData.forecast = weatherData.data.forecast.slice(0, 6); // Assuming you want the first 6 days
	return weatherData;
};

export const getWeeklyAvgMaxTempC = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.maxTempC, 0);
	return total / forecast.length;
};

export const getWeeklyAvgMinTempC = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.minTempC, 0);
	return total / forecast.length;
};

export const getWeeklyAvgTempC = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.avgTempC, 0);
	return total / forecast.length;
};

export const getWeeklyAvgMaxTempF = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.maxTempF, 0);
	return total / forecast.length;
};

export const getWeeklyAvgMinTempF = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.minTempF, 0);
	return total / forecast.length;
};

export const getWeeklyAvgTempF = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.avgTempF, 0);
	return total / forecast.length;
};

export const getWeeklyAvgWindSpeed = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.windSpeed, 0);
	return total / forecast.length;
};

export const getWeeklyAvgWindDir = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.windDir, 0);
	return total / forecast.length;
};

export const getWeeklyTotalPrecipitation = forecast => {
	return forecast.reduce((sum, entry) => sum + entry.precipitation, 0);
};

export const getWeeklyAvgHumidity = forecast => {
	const total = forecast.reduce((sum, entry) => sum + entry.humidity, 0);
	return total / forecast.length;
};

export const getWeeklyConditions = forecast => {
...
export const getWeeklyHighTempC = forecast => Math.max(...forecast.map(entry => entry.maxTempC));
export const getWeeklyLowTempC = forecast => Math.min(...forecast.map(entry => entry.minTempC));
