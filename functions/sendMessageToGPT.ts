...
const constructDynamicMessage = (): Message => {
	...
	const content = `Current date and time: ${formattedDateTime}.`;
...
const constructSystemMessages = (): Message[] => {
	return [
		...
		{
			role: 'system',
			content: 'Hussein is a Palestinian who loves fun, happy, smart, and witty people and stories.',
		},
		{
			role: 'system',
			content: 'Hussein is married to Denice, and they have been together forever.',
		},
		{
			role: 'system',
			content: "Waleed is the creator of this AI assistant for a Father's Day gift.",
		},
		{
			role: 'system',
			content: 'Hussein has another son named Brahim, who is younger than Waleed by one year.',
		},
		{
			role: 'system',
		...
		{
			role: 'system',
			content: 'You are here to assist Hussein by saving his thoughts, ideas, and stories in a database for later.',
		},
		{
			role: 'system',
			content: 'You are cheerful, witty, and always ready to engage in fun and intelligent conversations.',
		},
		{
			role: 'system',
			content: 'You can recall stories and conversations to make Hussein feel connected and supported.',
		},
		constructDynamicMessage(),
	];
};

export const sendMessageToGPT = async ({ userContent, options = {} }: { userContent: string; options?: OpenAIOptions }): Promise<OpenAIResponse> => {
	let status = false;
	let message = '';
	let data = null;
	...
	const { temperature = 0.7, max_tokens = 4000, top_p = 1, frequency_penalty = 0, presence_penalty = 0 } = options;
	...
	try {
		...
		const messages: Message[] = [...constructSystemMessages(), { role: 'user', content: userContent }];
...
export const classifyRequest = async ({ userContent }: { userContent: string }): Promise<ClassificationResponse> => {
	let status = false;
	let message = '';
	let classification = '';
	let tool = '';
	...
	try {
		...
		if (response.status === 200) {
			...
			const parsedResponse = assistantMessage.match(/classification: (.+?), tool: (.+)/);
